...
function Deployed() {
  ...
  const deployedTasks = tasks.filter(
    (task) => task.Status?.toLowerCase() === "deployed"
  );
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#1e1e2f" }} className="p-4">
      <h2 className="text-white mb-5 text-center fw-bold">Deployed Tasks</h2>
      ...
      {deployedTasks.length > 0 ? (
        ...
        <div className="d-flex flex-column justify-content-center align-items-center mt-5 py-5">
          <h4 className="text-white fw-semibold">No Deployed Tasks</h4>
          <p className="text-muted text-center w-50">
            You currently have no deployed tasks.
          </p>
             <button
