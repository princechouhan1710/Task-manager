...
function Deferred() {
  ...
  const deferredTasks = tasks.filter(
    (task) => task.Status?.toLowerCase() === "deferred"
  );
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#1e1e2f" }} className="p-4">
      <h2 className="text-white mb-5 text-center fw-bold">Deferred Tasks</h2>
      ...
      {deferredTasks.length > 0 ? (
        ...
        <div className="d-flex flex-column justify-content-center align-items-center mt-5 py-5">
          <h4 className="text-white fw-semibold">No Deferred Tasks</h4>
          <p className="text-muted text-center w-50">
            You currently have no deferred tasks.
          </p>
             <button
